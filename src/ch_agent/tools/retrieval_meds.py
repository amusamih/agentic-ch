...
def _simple_relevance_sections(text: str, query: str) -> List[str]:
    ...
    for sec in sections:
        ...
        for kw in keywords:
            if kw in sec_lower:
                score += 1
        if "interaction" in sec_lower or "safety" in sec_lower:
            score += 1
...
def retrieve_meds_guidance_tool(kb_path: Path, user_query: str) -> ToolResult:
    if not kb_path.exists():
        return ToolResult(tool_name="retrieve_meds_guidance", outputs={}, ok=False, error=f"KB not found: {kb_path}")
    ...
    return ToolResult(
        tool_name="retrieve_meds_guidance",
